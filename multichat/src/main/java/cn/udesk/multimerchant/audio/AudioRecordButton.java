...
public class AudioRecordButton extends AppCompatButton implements AudioRecordManager.OnAudioStateListener {
    ...
    public AudioRecordButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // 初始化按钮样式
        try {
            setBackgroundResource(R.drawable.udesk_multimerchant_record_button_normal);
            setText(getResources().getString(R.string.udesk_multimerchant_press_record));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
            ...
            try {
                switch (msg.what) {
                    case MSG_AUDIO_PREPARED:
                        ...
                        // 启动线程，每隔0.1秒获取音量大小
                        AsyncTask.execute(mGetVoiceLevelRunnable);
                        break;
        ...
        try {
            if (mCurrentState != state) {
                ...
                if (state == STATE_NORMAL) {
                    ...
                    setText(getResources().getString(R.string.udesk_multimerchant_release_end));
                    setBackgroundResource(R.drawable.udesk_multimerchant_record_button_recoding);
                    if (isRecording) {
                        mDialogManager.showRecording();
                    }
                } else if (state == STATE_WANT_CANCEL) {
                    setText(getResources().getString(R.string.udesk_multimerchant_release_cancel));
                    setBackgroundResource(R.drawable.udesk_multimerchant_record_button_recoding);
                    if (isRecording) {
